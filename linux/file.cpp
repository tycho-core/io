...
namespace io
{
	...
	{
		return file_ptr(new linux::file());
	}
...
namespace linux
{
	...
	int file::read(char* buffer, int size)
	{
		if(!m_f)
			return false;
		return ::fread(buffer, size, 1, m_f);
	}
	...
	bool file::seek_set(core::int64 pos)
	{
		if(!m_f)
			return false;
		return (::fseek(m_f, (long)pos, SEEK_SET) == 0) ? true : false;
	}
	...
	bool file::seek_end(core::int64 pos)
	{
		if(!m_f)
			return false;
		return (::fseek(m_f, (long)pos, SEEK_END) == 0) ? true : false;
	}
